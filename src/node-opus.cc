...
NodeOpusEncoder::NodeOpusEncoder(const CallbackInfo &args)
    : ObjectWrap<NodeOpusEncoder>(args) {
  this->encoder = nullptr;
  this->decoder = nullptr;
  this->outPcm = nullptr;

  if (args.Length() < 2) {
    Napi::RangeError::New(args.Env(), "Expected 2 arguments")
        .ThrowAsJavaScriptException();
    return;
  }

  if (!args[0].IsNumber() || !args[1].IsNumber()) {
    Napi::TypeError::New(args.Env(), "Expected rate and channels to be numbers")
        .ThrowAsJavaScriptException();
    return;
  }
...
Napi::Value NodeOpusEncoder::Encode(const CallbackInfo &args) {
  ...
  if (this->EnsureEncoder() != OPUS_OK) {
    Napi::Error::New(env,
                     "Could not create encoder. Check the encoder parameters")
        .ThrowAsJavaScriptException();
    return env.Null();
  }

  if (args.Length() < 1) {
    Napi::RangeError::New(env, "Expected 1 argument")
        .ThrowAsJavaScriptException();
    return env.Null();
  }

  if (!args[0].IsBuffer()) {
    Napi::TypeError::New(env, "Provided input needs to be a buffer")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  int compressedLength = opus_encode(this->encoder, pcm, frameSize,
                                     &(this->outOpus[0]), MAX_PACKET_SIZE);

  Buffer<char> actualBuf = Buffer<char>::Copy(
      env, reinterpret_cast<char *>(this->outOpus), compressedLength);

  if (!actualBuf.IsEmpty())
    return actualBuf;

  Napi::Error::New(env, "Could not encode the data")
      .ThrowAsJavaScriptException();
  return env.Null();
}
...
Napi::Value NodeOpusEncoder::Decode(const CallbackInfo &args) {
  ...
  if (this->EnsureDecoder() != OPUS_OK) {
    Napi::Error::New(env,
                     "Could not create decoder. Check the decoder parameters")
        .ThrowAsJavaScriptException();
    return env.Null();
  }

  int decodedSamples =
      opus_decode(this->decoder, compressedData, compressedDataLength,
                  &(this->outPcm[0]), MAX_FRAME_SIZE,
                  /* decode_fec */ 0);

  if (decodedSamples < 0) {
    Napi::TypeError::New(env, getDecodeError(decodedSamples))
        .ThrowAsJavaScriptException();
    return env.Null();
  }
  ...
  Buffer<char> actualBuf = Buffer<char>::Copy(
      env, reinterpret_cast<char *>(this->outPcm), decodedLength);

  if (!actualBuf.IsEmpty())
    return actualBuf;

  Napi::Error::New(env, "Could not decode the data")
      .ThrowAsJavaScriptException();
  return env.Null();
}
...
void NodeOpusEncoder::ApplyEncoderCTL(const CallbackInfo &args) {
  ...
  if (args.Length() < 2) {
    Napi::RangeError::New(env, "Expected 2 arguments")
        .ThrowAsJavaScriptException();
    return;
  }

  if (!args[0].IsNumber() || !args[1].IsNumber()) {
    Napi::TypeError::New(env, "Expected ctl and value to be numbers")
        .ThrowAsJavaScriptException();
    return;
  }
  ...
  if (opus_encoder_ctl(this->encoder, ctl, value) != OPUS_OK) {
    Napi::TypeError::New(env, "Invalid ctl / value")
        .ThrowAsJavaScriptException();
    return;
  }
}
...
void NodeOpusEncoder::SetBitrate(const CallbackInfo &args) {
  ...
  if (!args[0].IsNumber()) {
    Napi::TypeError::New(env, "Expected bitrate to be a number")
        .ThrowAsJavaScriptException();
    return;
  }
...
__attribute__((visibility("default"))) [[nodiscard]] Object
Init(Napi::Env env, Object exports) {
  return node_opus::NodeOpusEncoder::Init(env, exports);
}
